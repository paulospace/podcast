...
const fetchPodcastFeedRSS = async (feedURL) => {
    ...
    try {
        const res = await fetch(url).then(data => data.text()).then(data => xmlParser.parse(data));
...
const parseRSStoPodcastObject = (rssFeed) => {
    const podcast= {
        channel: {}, feed:[]
    }
    podcast.channel = rssFeed.channel;
    rssFeed.channel.item.forEach((item, index) => {
        console.log(item)
        podcast.feed[index] = item;
    })

    return podcast;
}

export {fetchPodcastFeedRSS, fetchItunesSearchPodcast, parseRSStoPodcastObject}